  ...
  public function build() {
    ...
    if ($entity->hasField('field_current_website')) {
      if ($url = $entity->field_current_website->uri) {
        $title = t('archived version of this advice');
        if ($entity->field_current_website->title) {
          $title = $entity->field_current_website->title;
        }
        return ['#markup' => '<div class="inset-text--beta"><p>' . t('This is our beta website. Pages are being tested and improved. You can view the <a href="@url">@title</a> on The National Archives website.', ['@url' => $url, '@title' => $title]) . '</p></div>'];
      }
    }
    return ['#markup' => ''];
  }
